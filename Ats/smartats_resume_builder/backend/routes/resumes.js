...
router.post('/:id/view', (req, res) => {
  try {
    const result = db.incrementViews(req.params.id, req.user.userId);
    
    if (!result) {
      return res.status(404).json({ error: 'Resume not found' });
    }

    res.json({ message: 'View tracked', views: result.views });
  } catch (error) {
...
router.post('/:id/download', (req, res) => {
  try {
    const result = db.incrementDownloads(req.params.id, req.user.userId);
    
    if (!result) {
      return res.status(404).json({ error: 'Resume not found' });
    }

    res.json({ message: 'Download tracked', downloads: result.downloads });
  } catch (error) {
...
router.post('/:id/application', (req, res) => {
  try {
    const result = db.incrementApplications(req.params.id, req.user.userId);
    
    if (!result) {
      return res.status(404).json({ error: 'Resume not found' });
    }

    res.json({ message: 'Application tracked', applications: result.applications });
  } catch (error) {
