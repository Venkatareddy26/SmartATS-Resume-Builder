...
export default function DashboardPage() {
  ...
  const handleCreateNew = () => {
    // Clear any flags - default shows sample resume
    localStorage.removeItem('smartats-create-blank')
    navigate('/editor/new')
  }

  const handleCreateBlank = () => {
    // Set flag to create blank resume
    localStorage.setItem('smartats-create-blank', 'true')
    // Clear any existing draft to ensure blank resume shows
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row gap-4">
            <button onClick={handleCreateNew} className="btn-primary flex items-center justify-center gap-2">
              <Plus className="w-5 h-5" />
              Create New Resume (with Sample)
            </button>
            <button onClick={handleCreateBlank} className="btn-secondary flex items-center justify-center gap-2">
              <FileText className="w-5 h-5" />
              Create Blank Resume
            </button>
          </div>
          <p className="text-xs text-slate-500 mt-2">
            💡 Tip: Start with a sample resume to see what a complete resume looks like, then customize it!
          </p>
        </div>
